...
def page_overview(df: pd.DataFrame):
    ...
    with col_main:
        ...
        c3.metric(
            "NLP Risk Score",
            f"{nlp_val:.3f}",
            help="Derived from merchant description text using TF-IDF + Random Forest."
        )
        ...
        with col_exp:
            ...
            # NLP interpretation (caption + reason)
            if nlp_val >= 0.7:
                st.caption("🔴 High-risk industry keywords detected (crypto / betting / gaming)")
                reasons.append(
                    f"🔤 Description matches high-risk industries (NLP Score: {nlp_val:.2f})"
                )
            elif nlp_val >= 0.3:
                st.caption("🟠 Moderate-risk commercial language detected")
                reasons.append(
                    f"🔤 Description shows moderate-risk business patterns (NLP Score: {nlp_val:.2f})"
                )
            else:
                st.caption("🟢 Neutral or low-risk business description")
...
def page_add_transaction(df: pd.DataFrame):
    ...
    with col_form:
        ...
        mid_suffix  = st.text_input("MerchantID Number", placeholder="000001", help="Enter numbers only. 'MID' will be prefixed automatically.")
        
        # Check if merchant exists dynamically to determine category input
        temp_mid = f"MID{mid_suffix}" if mid_suffix else ""
        merchant_exists = (df["MerchantID"] == temp_mid).any()
        
        if merchant_exists:
            old_cat = df[df["MerchantID"] == temp_mid].iloc[-1].get("Category", "Unknown")
            st.info(f" Existing Merchant detected. Category: **{old_cat}**")
            category = old_cat
        else:
        ...
        refund_rate_pct = st.slider("Refund Rate (%)", 0.0, 100.0, 2.0, 0.1, format="%.1f")
        cb_rate_pct     = st.slider("Chargeback Rate (%)", 0.0, 100.0, 0.5, 0.1, format="%.1f")
        
        # Convert percentages back to real rates (0.0 to 1.0) for internal processing
        refund_rate = refund_rate_pct / 100.0
        cb_rate     = cb_rate_pct / 100.0
    ...
    if submitted:
        if not mid_suffix.isdigit():
            st.error(" MerchantID Number must contain only digits.")
        elif not description.strip():
            st.error("Description cannot be empty.")
        elif len(mid_suffix) < 6:
            st.error("❌ MerchantID Number must be at least 6 digits long.")
        elif len(description.strip()) < 5:
            st.error(" Description must be at least 5 characters long.")
        else:
            mid = f"MID{mid_suffix}"
            ...
            from data_generation import is_festival
            ...
            if existing_mask.any():
                old_row = df[existing_mask].iloc[-1]
                old_txn_count = float(old_row.get("TransactionCount", 0))
                old_volume = float(old_row.get("TotalVolume", 0))
                old_refund_rate = float(old_row.get("RefundRate", 0))
                old_cb_rate = float(old_row.get("ChargebackRate", 0))

                new_txn_count = old_txn_count + txn_count
                new_volume = old_volume + volume
                new_refund_rate = (old_refund_rate + refund_rate) / 2.0
                new_cb_rate = (old_cb_rate + cb_rate) / 2.0
            else:
                new_txn_count = txn_count
                new_volume = volume
                new_refund_rate = refund_rate
                new_cb_rate = cb_rate

            # Compute derived features
            avg_txn_val = new_volume / max(new_txn_count, 1)
            nlp_score   = compute_nlp_risk_score(description)
            month_sin   = np.sin(2 * np.pi * month / 12)
            month_cos   = np.cos(2 * np.pi * month / 12)
            
            festival_flag = int(is_festival(month, ts_full.day))

            new_row = pd.DataFrame([{
                "MerchantID":         mid,
                "TransactionCount":   new_txn_count,
                "TotalVolume":        new_volume,
                "RefundRate":         new_refund_rate,
                "ChargebackRate":     new_cb_rate,
                "Category":           category,
                "Description":        description,
                "Timestamp":          ts_full,
                "is_festival_period": festival_flag,
            ...
            with col_result:
                ...
                with c2:
                    ...
                    if new_cb_rate > 0.10:
                        st.warning(f" Chargeback rate {new_cb_rate*100:.1f}% exceeds threshold!")
                    if nlp_score > 0.30:
...
def page_trends(df: pd.DataFrame):
    ...
    # ── Category-wise ─────────────────────────────────────────────────────────
    st.subheader(" Category-wise Transaction Volume")
...
def page_plots(df: pd.DataFrame):
    ...
    st.subheader(" Category × Risk Level Heatmap")
